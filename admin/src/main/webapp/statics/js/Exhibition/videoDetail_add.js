    ...
    initTable: function () {
        ...
        layui.use('form', function(){
            ...
            form.on('select(saveType)', function(data){
                ...
                fromSelects.data('select2', 'server', {
                    url: property.getProjectPath() + "PostVideo/getPostVideoForExhib.do?saveType="+$('#saveType').val()
                })
            });
        })
        ...
        layui.use('form', function () {
            ...
            $.get(projectName + 'interfaceCollect/getCollctTypeList.do', function (res) {
                if (res.success == 1) {
                    ...
                    var culCategorySelect  = component.getSelectSimplePlus(data,data[0].typeCode,"culCategory","typeCode","typename");
            ...
            var saveTypeSelect  = component.getSelectSimplePlus( videoSaveTypeList,videoSaveTypeList[0].dictCode,"saveType","dictCode","dictName");
            $("#saveType").append(saveTypeSelect);
            form.render("select");
            formSelects.data('select1', 'server', {
                url:property.getProjectPath()+"interfaceCollect/getCollectByTypeAndName.do?culCategory="+videoSaveTypeList[0].dictCode
            });
            fromSelects.data('select2', 'server', {
                url: property.getProjectPath() + "PostVideo/getPostVideoForExhib.do?saveType="+videoSaveTypeList[0].dictCode
            })
            //监听提交
            form.on('submit(formDemo)', function (data) {
                ...
                var async = false;
                var datas = yc.ajaxPostByJson('exhib/addExhibition', exhibitionDto,async,"添加展陈成功",data.elem);
        ...
        layui.use(['upload', 'element','form'], function () {
            ...
            var demoListView = $('#demoList')
                ...
                elem: '#test10'
                ,url: property.getProjectPath()+"attach/upload.do?tableName="+"exhibition"+"&tableId="+tableId
                , accept: 'file'
                , multiple: true
                , auto: false
                , xhr: xhrOnProgress
