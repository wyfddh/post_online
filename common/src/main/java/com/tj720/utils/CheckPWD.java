...
public class CheckPWD {
    ...
    public static boolean checkPasswordLength(String password) {
        ...
        if("".equals(CheckConfig.MAX_LENGTH)) {
            if (password.length() >= Integer.parseInt(CheckConfig.MIN_LENGTH)) {
                flag = true;
            }
        }else{
            if (password.length() >= Integer.parseInt(CheckConfig.MIN_LENGTH) &&
                    password.length() <= Integer.parseInt(CheckConfig.MAX_LENGTH)) {
                flag = true;
            }
        }
        return flag;
    ...
    public static boolean checkContainSpecialChar(String password) {
        ...
        for (int i = 0; i < chPass.length; i++) {
            if (CheckConfig.SPECIAL_CHAR.indexOf(chPass[i]) != -1) {
                special_count++;
            }
        }
    ...
    public static boolean checkLateralKeyboardSite(String password) {
         ...
         */
        boolean flag = false;
        int arrLen = CheckConfig.KEYBOARD_HORIZONTAL_ARR.length;
        int limit_num = Integer.parseInt(CheckConfig.LIMIT_HORIZONTAL_NUM_KEY) ;
        ...
        for(int i=0; i+limit_num<=n; i++) {
            ...
            for(int j=0; j<arrLen; j++) {
                String configStr = CheckConfig.KEYBOARD_HORIZONTAL_ARR[j];
                String revOrderStr = new StringBuffer(CheckConfig.KEYBOARD_HORIZONTAL_ARR[j]).reverse().toString();

                //检测包含字母(区分大小写)
                if ("enable".equals(CheckConfig.CHECK_DISTINGGUISH_CASE)) {
                    //考虑 大写键盘匹配的情况
                    String UpperStr = CheckConfig.KEYBOARD_HORIZONTAL_ARR[j].toUpperCase();
    ...
    public static boolean checkKeyboardSlantSite(String password) {
         ...
         */
        boolean flag = false;
        int arrLen = CheckConfig.KEYBOARD_SLOPE_ARR.length;
        int limit_num = Integer.parseInt(CheckConfig.LIMIT_SLOPE_NUM_KEY);

        for(int i=0; i+limit_num<=n; i++) {
            String str = t_password.substring(i, i+limit_num);
            String distinguishStr = password.substring(i, i+limit_num);
            for(int j=0; j<arrLen; j++) {
                String configStr = CheckConfig.KEYBOARD_SLOPE_ARR[j];
                String revOrderStr = new StringBuffer(CheckConfig.KEYBOARD_SLOPE_ARR[j]).reverse().toString();
                //检测包含字母(区分大小写)
                if ("enable".equals(CheckConfig.CHECK_DISTINGGUISH_CASE)) {

                    //考虑 大写键盘匹配的情况
                    String UpperStr = CheckConfig.KEYBOARD_SLOPE_ARR[j].toUpperCase();
                    if((configStr.indexOf(distinguishStr) != -1) || (UpperStr.indexOf(distinguishStr) != -1)) {
                        flag = true;
                        return flag;
                    }
    ...
    public static boolean checkSequentialChars(String password) {
        String t_password = new String(password);
        boolean flag = false;
        int limit_num = Integer.parseInt(CheckConfig.LIMIT_LOGIC_NUM_CHAR);
        int normal_count = 0;
    ...
    public static boolean checkSequentialSameChars(String password) {
        ...
        char[] pwdCharArr = t_password.toCharArray();
        boolean flag = false;
        int limit_num = Integer.parseInt(CheckConfig.LIMIT_NUM_SAME_CHAR);
        int count = 0;
        for (int i=0; i+limit_num<=n; i++) {
            count=0;
            for (int j=0; j<limit_num-1; j++) {
    ...
    public static boolean EvalPWD(String password) {
         ...
         * 检测长度
         */
        if ("enable".equals(CheckConfig.CHECK_PASSWORD_LENGTH)){
         ...
         * 检测包含数字
         */
        if ("enable".equals(CheckConfig.CHECK_CONTAIN_DIGIT)){
        ...
        if ("enable".equals(CheckConfig.CHECK_DISTINGGUISH_CASE)){
            //检测包含小写字母
            if ("enable".equals(CheckConfig.CHECK_LOWER_CASE)){
            ...
            //检测包含大写字母
            if ("enable".equals(CheckConfig.CHECK_UPPER_CASE)){
         ...
         * 检测包含特殊符号
         */
        if ("enable".equals(CheckConfig.CHECK_CONTAIN_SPECIAL_CHAR)){
         ...
         * 检测键盘横向连续
         */
        if ("enable".equals(CheckConfig.CHECK_HORIZONTAL_KEY_SEQUENTIAL)){
         ...
         * 检测键盘斜向连续
         */
        if ("enable".equals(CheckConfig.CHECK_SLOPE_KEY_SEQUENTIAL)){
         ...
         * 检测逻辑位置连续
         */
        if ("enable".equals(CheckConfig.CHECK_LOGIC_SEQUENTIAL)){
         ...
         * 检测相邻字符是否相同
         */
        if ("enable".equals(CheckConfig.CHECK_SEQUENTIAL_CHAR_SAME)){
